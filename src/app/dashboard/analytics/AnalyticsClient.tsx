...
// Build chart data from postAnalyticsList — group by date, sum likes + comments
function buildChartData(postAnalyticsList: AnalyticsData["postAnalyticsList"]) {
  const map: Record<string, { date: string; likes: number; comments: number }> =
    {};

  for (const post of postAnalyticsList) {
    if (!post.analytics || !post.publishedAt) continue;
    const date = post.publishedAt.split("T")[0];
    if (!map[date]) {
      map[date] = { date, likes: 0, comments: 0 };
    }
    map[date].likes += post.analytics.likes;
    map[date].comments += post.analytics.comments;
  }
...
export default function AnalyticsClient() {
  ...
  const chartData = data ? buildChartData(data.postAnalyticsList) : [];
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-6 py-12 text-white">
      <section className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-400 flex items-center gap-2">
          ...
          Meta app is in <span className="font-semibold">
            development mode
          </span>{" "}
