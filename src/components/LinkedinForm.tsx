...
export default function LinkedinForm() {
  ...
  const [docAudienceRole, setDocAudienceRole] = useState("");
  const [docTone, setDocTone] = useState("professional");
  const [docPostGoal, setDocPostGoal] = useState("thought-leadership");
  ...
  async function generateFromDocument() {
    ...
    if (!docAudienceRole) {
      alert("Please fill in the target audience");
      return;
    }
    setIsUploading(true);
    setIsGenerating(true);
    ...
    try {
      ...
      const genRes = await fetch("/api/generate-post-from-document", {
        ...
        body: JSON.stringify({
          ...
          tone: docTone,
          platform,
          targetAudience: docAudienceRole,
          postGoal: docPostGoal,
        }),
      });
            ...
            >
              ...
              <div className="rounded-2xl bg-slate-900/80 border border-slate-800 p-8">
                ...
                <p className="text-slate-400 text-sm mb-6">
                  Upload a PDF, DOCX, or TXT file — AI will generate LinkedIn
              ...
              <div className="rounded-2xl bg-slate-900/80 border border-slate-800 p-8 space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      value={docAudienceRole}
                      onChange={(e) => setDocAudienceRole(e.target.value)}
                      className="w-full rounded-xl bg-slate-950 border border-slate-800 px-4 py-3"
                      placeholder="Recruiters, founders, engineers"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      value={docPostGoal}
                      onChange={(e) => setDocPostGoal(e.target.value)}
                      className="w-full rounded-xl bg-slate-950 border border-slate-800 px-4 py-3"
                    >
                      <option value="thought-leadership">
                        Thought Leadership
